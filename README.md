...
- **Session sync** — Import Claude Code and Codex CLI sessions with a default maximum of 10 per `/sync-sessions` run. Both native stores are discovered automatically (`~/.claude/projects` and `$CODEX_HOME/sessions`, normally `~/.codex/sessions`); `/sync-settings` changes thread style, time window, minimum results, and the configurable batch maximum
