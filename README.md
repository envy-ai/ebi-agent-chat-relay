...
- **Queued instructions** — `/queue command:<instruction>` appends work to the current thread's FIFO queue. Queued items stack, wait for the active turn without interrupting it, and then continue the latest saved session; when the thread is idle, the first item starts immediately
