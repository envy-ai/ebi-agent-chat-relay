...
- **Session sync** — Import Claude Code and Codex CLI sessions with a default maximum of 10 per `/sync-sessions` run. Pass `/sync-sessions session_id:<id>` to import one exact Codex session regardless of its age or the configured batch maximum. Both native stores are discovered automatically (`~/.claude/projects` and `$CODEX_HOME/sessions`, normally `~/.codex/sessions`); `/sync-settings` changes thread style, time window, minimum results, and the configurable batch maximum
- **Queued instructions** — `/queue command:<instruction>` appends work to the current thread's FIFO queue. Queued items stack, wait for the active turn without interrupting it, and then continue the latest saved session; when the thread is idle, the first item starts immediately. Plain-text `/queue <instruction>` is accepted as a fallback when Discord sends it as a regular message instead of invoking the registered slash command
