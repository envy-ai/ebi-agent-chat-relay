...
- **Session sync** — Import up to 10 Claude Code and Codex CLI sessions per `/sync-sessions` run. Both native stores are discovered automatically (`~/.claude/projects` and `$CODEX_HOME/sessions`, normally `~/.codex/sessions`); `/sync-settings` changes thread style, time window, and minimum results
